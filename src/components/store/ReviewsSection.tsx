      ...
      {[1,2,3,4,5].map(s => (
        <span key={s}
          ...
          style={{
            ...
            color: s <= (hover || rating) ? "#C9CFD4" : "#262626",
  ...
  return (
    <div style={{ borderTop: "1px solid rgba(155,163,170,0.12)", paddingTop: 64, marginTop: 64, direction: "rtl" }}>
      <style>{`
        ...
        .rev-card:hover { border-color:rgba(155,163,170,0.18); }
      `}</style>
      ...
      {reviewsList.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 32 }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "Tajawal,sans-serif", fontSize: 48, fontWeight: 900, color: "#9BA3AA", lineHeight: 1 }}>
              {avg.toFixed(1)}
            </div>
            ...
            {[5,4,3,2,1].map(s => {
              ...
              return (
                <div key={s} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                  <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 11, color: "#F2F0EC", opacity: 0.4, minWidth: 8 }}>{s}</span>
                  <span style={{ color: "#C9CFD4", fontSize: 10 }}>★</span>
                  <div style={{ flex: 1, height: 5, background: "#1A1A1A", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg,#9BA3AA,#C9CFD4)", borderRadius: 3, transition: "width 0.6s ease" }} />
                  </div>
                  <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 10, color: "#F2F0EC", opacity: 0.3, minWidth: 16 }}>{count}</span>
                </div>
              )
            })}
          </div>
      ...
      <div style={{ display: "flex", gap: 40, flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <div style={{ flex: "1 1 400px" }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "32px 0" }}>
              ...
              <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 14, color: "#F2F0EC", opacity: 0.3 }}>
              ...
              {reviewsList.map(r => (
                <div key={r.id} className="rev-card">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    <div>
                      <span style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 700, fontSize: 14, color: "#F2F0EC" }}>{r.customer_name}</span>
                      <div style={{ marginTop: 3 }}>
                        <StarRating rating={r.rating} />
                      </div>
                    </div>
                    <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 11, color: "#F2F0EC", opacity: 0.25 }}>
                      {new Date(r.created_at).toLocaleDateString("ar-EG")}
                    </span>
                  </div>
                  {r.comment_ar && (
                    <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 14, color: "#F2F0EC", opacity: 0.6, lineHeight: 1.8, margin: 0 }}>
                      {r.comment_ar}
                    </p>
                  )}
                </div>
        ...
        <div style={{ flex: "0 0 320px", minWidth: 260 }}>
          <div style={{ background: "linear-gradient(145deg,#131313,#141414)", border: "1px solid rgba(155,163,170,0.1)", borderRadius: 16, padding: "24px 20px" }}>
            <h3 style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 700, fontSize: 15, color: "#F2F0EC", marginBottom: 20, borderBottom: "1px solid rgba(155,163,170,0.08)", paddingBottom: 12 }}>
            ...
            {submitted ? (
              <div style={{ textAlign: "center", padding: "16px 0" }}>
                ...
                <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 14, color: "#F2F0EC", opacity: 0.55, lineHeight: 1.8 }}>
                  تم إرسال تقييمك بنجاح وسيظهر بعد المراجعة.
                </p>
                <button onClick={() => setSubmitted(false)}
                  style={{ marginTop: 16, background: "none", border: "1px solid rgba(155,163,170,0.2)", color: "#9BA3AA", fontFamily: "Tajawal,sans-serif", fontSize: 12, padding: "7px 16px", borderRadius: 8, cursor: "pointer" }}>
                  إضافة تقييم آخر
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {/* Stars */}
                <div>
                  <label style={{ display: "block", fontFamily: "Tajawal,sans-serif", fontSize: 12, color: "#F2F0EC", opacity: 0.5, marginBottom: 8 }}>تقييمك *</label>
                  <StarRating rating={rating} interactive onRate={setRating} />
                </div>
                <div>
                  <label style={{ display: "block", fontFamily: "Tajawal,sans-serif", fontSize: 12, color: "#F2F0EC", opacity: 0.5, marginBottom: 6 }}>اسمك *</label>
                  <input className="rev-input" required placeholder="مثال: سارة م." value={name} onChange={e => setName(e.target.value)} />
                </div>
                <div>
                  <label style={{ display: "block", fontFamily: "Tajawal,sans-serif", fontSize: 12, color: "#F2F0EC", opacity: 0.5, marginBottom: 6 }}>تعليقك (اختياري)</label>
                ...
                <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 10, color: "#F2F0EC", opacity: 0.2, textAlign: "center" }}>
