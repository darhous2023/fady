  ...
  return (
    <div style={{ marginTop: 60, direction: "rtl" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        <div style={{ height: 1, flex: 1, background: "linear-gradient(90deg,transparent,rgba(201,168,76,0.15))" }} />
        <span style={{ fontFamily: "Cinzel,serif", fontSize: 9, letterSpacing: "5px", color: "#C9A84C", opacity: 0.6, textTransform: "uppercase", whiteSpace: "nowrap" }}>
          شاهدتِ مؤخراً
        </span>
        <div style={{ height: 1, flex: 1, background: "linear-gradient(90deg,rgba(201,168,76,0.15),transparent)" }} />
      </div>
        ...
        {items.map(item => (
          <Link key={item.id} href={`/products/${item.slug}`} style={{ textDecoration: "none", flexShrink: 0, width: 130 }}>
            <div style={{
              ...
              border: "1px solid rgba(201,168,76,0.1)", borderRadius: 12, overflow: "hidden",
              transition: "all 0.25s ease",
            }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(201,168,76,0.3)"; e.currentTarget.style.transform = "translateY(-2px)" }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(201,168,76,0.1)"; e.currentTarget.style.transform = "" }}>
              ...
              <div style={{ padding: "10px 10px 12px" }}>
                <div style={{ fontFamily: "Tajawal,sans-serif", fontSize: 12, fontWeight: 700, color: "#F5EFE0", lineHeight: 1.4, marginBottom: 4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  {item.name_ar}
                </div>
                <div style={{ fontFamily: "Tajawal,sans-serif", fontSize: 13, fontWeight: 900, color: "#C9A84C" }}>
