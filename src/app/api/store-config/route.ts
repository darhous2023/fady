...
export async function GET() {
  try {
    const rows = await db.select().from(settings).where(
      sql`${settings.key} IN ('whatsapp_number','store_name_ar','store_tagline_ar','instagram_showroom_url','instagram_manager_url','facebook_url','maps_url','address_ar','intro_tagline_ar')`
    )
