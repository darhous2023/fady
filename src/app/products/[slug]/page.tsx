...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    console.error(`[products/${slug}] generateMetadata failed:`, err)
    return { title: "سيارة" }
  }
  if (!rows[0]) return { title: "المنتج غير موجود" }

  let firstImg: { url: string } | undefined
  try {
    ;[firstImg] = await db
    ...
    console.error(`[products/${slug}] generateMetadata image lookup failed:`, err)
  }
...
function ProductLoadError() {
  return (
    <>
      <StoreHeader />
      <StoreErrorState message="تعذر تحميل بيانات هذه السيارة حاليًا. يرجى المحاولة مرة أخرى بعد قليل." minHeight="60vh" />
      <StoreFooter />
      <FloatingWA />
    </>
  )
...
export default async function ProductPage({ params }: Props) {
  ...
  try {
    ...
    console.error(`[products/${slug}] main query failed:`, err)
  ...
  let imgs, frames360, relatedRows, variantRows, pageSettings, relatedImages
  try {
    ;[imgs, frames360, relatedRows, variantRows, pageSettings] = await Promise.all([
    ...
    relatedImages = relatedIds.length
      ? await db
    ...
    console.error(`[products/${slug}] related-data query failed:`, err)
    return <ProductLoadError />
  }
