...
export default function CartPage() {
  ...
  return (
    <>
      <StoreHeader />
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@300;400;700;900&family=Cinzel:wght@400&family=Playfair+Display:wght@700&display=swap');
        * { box-sizing: border-box; }
        body { margin: 0; background: #0A0806; }
        .cart-item { transition: all 0.3s ease; }
        .cart-item:hover { border-color: rgba(201,168,76,0.25)!important; }
        .cart-qty-btn { background: rgba(201,168,76,0.08); border: 1px solid rgba(201,168,76,0.2); color: #C9A84C; border-radius: 6px; width: 32px; height: 32px; cursor: pointer; font-size: 16px; font-weight: 700; transition: all 0.2s; display: flex; align-items: center; justify-content: center; }
        .cart-qty-btn:hover { background: rgba(201,168,76,0.18); border-color: rgba(201,168,76,0.5); }
        ...
        .cart-remove-btn:hover { color: #c0392b; background: rgba(192,57,43,0.1); }
        .cart-checkout-btn:hover { opacity: 0.92; transform: translateY(-1px); box-shadow: 0 16px 48px rgba(201,168,76,0.4)!important; }
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 80px" }}>
          ...
          {items.length === 0 ? (
            ...
            <div style={{ textAlign: "center", padding: "80px 24px" }}>
              ...
              <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 18, color: "#F5EFE0", opacity: 0.4, marginBottom: 32 }}>
                سلتك فارغة حتى الآن
              </p>
            ...
            <div style={{ display: "flex", gap: 32, flexWrap: "wrap", alignItems: "flex-start" }}>
              ...
              <div style={{ flex: "1 1 500px", display: "flex", flexDirection: "column", gap: 14 }}>
                {items.map(item => (
                  <div key={item.id} className="cart-item" style={{
                    ...
                    {/* Image */}
                    <div style={{ width: 72, height: 72, borderRadius: 10, overflow: "hidden", flexShrink: 0, background: "#1a1510" }}>
                      {item.image ? (
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <Link href={`/products/${item.slug}`} style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 700, fontSize: 15, color: "#F5EFE0", textDecoration: "none", display: "block", marginBottom: 4 }}>
                        {item.name_ar}
                      </Link>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 700, fontSize: 16, color: "#F5EFE0", minWidth: 24, textAlign: "center" }}>
                        {item.quantity}
                      </span>
                      <button className="cart-qty-btn" onClick={() => updateQty(item.id, item.quantity + 1)}>+</button>
                    </div>
                    ...
                    {/* Subtotal */}
                    <div style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 900, fontSize: 15, color: "#C9A84C", minWidth: 90, textAlign: "center" }}>
              ...
              <div style={{ flex: "0 0 280px", minWidth: 240, position: "sticky", top: 88 }}>
                <div style={{
                  ...
                  <h3 style={{ fontFamily: "Tajawal,sans-serif", fontWeight: 700, fontSize: 16, color: "#F5EFE0", marginBottom: 20, borderBottom: "1px solid rgba(201,168,76,0.1)", paddingBottom: 14 }}>
                    ملخص الطلب
                  </h3>

                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 14, color: "#F5EFE0", opacity: 0.5 }}>الإجمالي ({count} منتج)</span>
                    <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 14, color: "#F5EFE0", fontWeight: 700 }}>{total.toLocaleString("ar-EG")} ج.م</span>
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
                    <span style={{ fontFamily: "Tajawal,sans-serif", fontSize: 15, fontWeight: 700, color: "#F5EFE0" }}>المجموع</span>
                    <span style={{
                  ...
                  <p style={{ fontFamily: "Tajawal,sans-serif", fontSize: 11, color: "#F5EFE0", opacity: 0.25, textAlign: "center", marginTop: 16 }}>
                    🔒 تسوق آمن — نحن معك
                  </p>
                </div>
              </div>
